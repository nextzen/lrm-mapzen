(function (factory) {
	...
	L.Control.Geocoder.Mapzen = L.Class.extend({
		...
		geocode: function(query, cb, context) {
			L.Control.Geocoder.getJSON(this.options.serviceUrl + 'search', {
				text: query,
				api_key: this._key
			},
			function(data) {
		...
		reverse: function(location, scale, cb, context) {
			L.Control.Geocoder.getJSON(this.options.serviceUrl + 'reverse', {
					'point.lat': location.lat,
					'point.lon': location.lng,
					api_key: this._key
				} , function(data) {
				var firstData = data.features[0];
				var result = [],
				    loc;
				if (firstData && firstData.geometry.coordinates) {
					loc = L.latLng(firstData.geometry.coordinates[1], firstData.geometry.coordinates[0]);
					result.push({
						name: firstData.properties.label,
						center: loc,
						properties: firstData.properties
					});
				}
	...
	L.Control.Geocoder.mapzen = function(key) {
		return new L.Control.Geocoder.Mapzen(key);
	}
